...
{
    public class NewUserHandler : IRequestHandler<NewUserCommand, Result<User>>
    {
        ...
        public NewUserHandler(DeckDbContext context, IMapper mapper)//, IValidator<NewUserCommand> validator)
        {
            _mapper = mapper;
            //_validator = validator;
            _context = context;
        }
        public async Task<Result<User>> Handle(NewUserCommand request, CancellationToken cancellationToken)
        {
            //validate
            ...
            try
            {
                var userDb = new UserDb(request.Name,request.Country);
