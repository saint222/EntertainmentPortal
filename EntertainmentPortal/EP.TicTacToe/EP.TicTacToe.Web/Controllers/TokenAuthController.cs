...
namespace EP.TicTacToe.Web.Controllers
{
    ...
    public class TokenAuthController : ControllerBase
    {
        ...
        public TokenAuthController(UserManager<IdentityUser> manager,
        ...
        private string GetToken(IdentityUser userData)
        {
            var key =
                new SymmetricSecurityKey(
                    Encoding.UTF8.GetBytes(TicTacToeConstants.SECRET));
            ...
            [FromBody] UserAuthData userAuthData)
        {
            ...
            {
                var status =
                    await _manager.CheckPasswordAsync(user, userAuthData.Password);
                if (status)
                {
